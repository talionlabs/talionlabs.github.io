...
═══════════════════════════════════════════════════════════ */
const VER         = '3.0.0';
const HOME_URL    = 'https://taliontype.is-a.dev';
const GUMROAD_URL = 'https://taliontype.is-a.dev'; // legacy, replaced by Stripe
const MAX_LEVEL   = 100;
...
// Language override: language pages set window.TT_LANG before loading this script
// { words: { easy: [...], medium: [...], hard: [...] }, name: 'Hindi', code: 'hi' }
if (window.TT_LANG && window.TT_LANG.words) {
  Object.assign(WORDS, window.TT_LANG.words);
}
...
function buildShareText(r) {
  r = r || S.lastResult;
  if (!r || r.wpm == null) return 'Check out TalionType — free typing speed test!\nhttps://taliontype.is-a.dev';
  // Calculate percentile from leaderboard history
  const hist = getHistory();
  let percentile = 50;
  if (hist.length >= 5) {
    const below = hist.filter(h => h.wpm < r.wpm).length;
    percentile = Math.round((below / hist.length) * 100);
  } else if (S.fbDb) {
    // Use a rough estimate based on WPM benchmarks
    if (r.wpm >= 120) percentile = 99;
    else if (r.wpm >= 100) percentile = 97;
    else if (r.wpm >= 80)  percentile = 92;
    else if (r.wpm >= 60)  percentile = 80;
    else if (r.wpm >= 40)  percentile = 55;
    else percentile = 30;
  }
  const pLine = percentile >= 50 ? `⚡ Faster than ${percentile}% of typists!\n` : '';
  return `⌨ I just typed ${r.wpm} WPM with ${r.acc}% accuracy on TalionType!\n`
       + pLine
       + `${r.errors === 0 ? '✨ Zero errors!\n' : ''}Mode: ${r.mode} / ${r.diff}\n\n`
       + `Challenge me → https://taliontype.is-a.dev`;
}
...
/* ═══════════════════════════════════════════════════════════
   ...
   2. Go to Payment Links → Create Link
      - Monthly: $3.99/month, 7-day free trial
        Success URL: https://taliontype.is-a.dev/?pro_success=1&email={CUSTOMER_EMAIL}
      - Yearly: $29/year, 7-day free trial
        Success URL: https://taliontype.is-a.dev/?pro_success=1&email={CUSTOMER_EMAIL}
   3. Replace STRIPE_MONTHLY_URL and STRIPE_YEARLY_URL below with your Payment Link URLs
   4. For webhook (auto-verify cancellations, optional):
      Use Zapier Free: Stripe → Webhook → Firebase Firestore write
═══════════════════════════════════════════════════════════ */
const STRIPE_MONTHLY_URL = 'https://buy.stripe.com/REPLACE_MONTHLY'; // ← paste your Stripe Payment Link
const STRIPE_YEARLY_URL  = 'https://buy.stripe.com/REPLACE_YEARLY';  // ← paste your Stripe Payment Link
...
// Track selected plan in modal
let _selectedProPlan = 'monthly';

function loadPro() {
  // Check localStorage first
  S.isPro = localStorage.getItem('tt_pro') === 'true';

  // Also check if signed-in user has pro in Firestore
  if (!S.isPro && S.fbDb && S.currentUser) {
    S.fbDb.collection('pro_subscribers').doc(S.currentUser.email?.toLowerCase()).get()
      .then(doc => {
...
function renderProModal() {
  ...
  if (S.isPro) {
    if (heroEl) heroEl.innerHTML = `
      ...
      <a href="https://billing.stripe.com/p/login/REPLACE_PORTAL" target="_blank" rel="noopener" class="ctrl-btn small" style="margin-top:.75rem;display:inline-flex">Manage Subscription →</a>`;
    if (actionsEl) actionsEl.style.display = 'none';
  } else {
    // Wire plan cards
    setTimeout(() => {
      document.querySelectorAll('.pro-plan-card').forEach(card => {
        card.addEventListener('click', () => {
          document.querySelectorAll('.pro-plan-card').forEach(c => c.classList.remove('pro-plan-active'));
          card.classList.add('pro-plan-active');
          _selectedProPlan = card.dataset.plan;
        });
      });
    }, 50);
...
function handleBuyPro(e) {
  e.preventDefault();
  const url = _selectedProPlan === 'yearly' ? STRIPE_YEARLY_URL : STRIPE_MONTHLY_URL;
  if (url.includes('REPLACE')) {
    showToast('⚙ Stripe not configured yet — add your Payment Link URLs to typing.js');
    return;
  }
  // If user is signed in, pre-fill their email in Stripe via query param
  const emailParam = S.currentUser?.email ? `?prefilled_email=${encodeURIComponent(S.currentUser.email)}` : '';
  window.open(url + emailParam, '_blank');
  gaEvent('pro_checkout', { plan: _selectedProPlan });
}

function verifyProKey() {
  const email = (D.proKeyInput?.value || '').trim().toLowerCase();
  if (!email || !email.includes('@')) {
    if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--err)'; D.proVerifyNote.textContent = 'Enter the email you used to subscribe.'; }
    return;
  }

  // Check stored email (set by Stripe success redirect)
  const storedEmail = localStorage.getItem('tt_pro_email') || '';
  if (storedEmail === email) {
    _unlockPro(); return;
  }

  // Check Firebase Firestore for subscriber record
  if (S.fbDb) {
    if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--text-m)'; D.proVerifyNote.textContent = 'Checking…'; }
    S.fbDb.collection('pro_subscribers').doc(email).get()
      .then(doc => {
        if (doc.exists && doc.data()?.active) {
          localStorage.setItem('tt_pro_email', email);
          _unlockPro();
        } else {
          if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--err)'; D.proVerifyNote.textContent = 'No active subscription found. Check your email or start a free trial above.'; }
        }
      }).catch(() => {
        if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--err)'; D.proVerifyNote.textContent = 'Could not verify — please try again.'; }
      });
  } else {
    if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--err)'; D.proVerifyNote.textContent = 'Please sign in first, then restore Pro.'; }
  }
}
...
function _unlockPro() {
  ...
  S.isPro = true;
  applyProUI();
  if (D.proVerifyNote) { D.proVerifyNote.style.color = 'var(--ok)'; D.proVerifyNote.textContent = '✓ Pro restored! Thank you for supporting TalionType.'; }
  showToast('⚡ Pro unlocked!');
  setTimeout(() => renderProModal(), 800);
  gaEvent('pro_restored');
}
...
function bindEvents() {
  ...
  // Wire buy button (it's an <a> but we intercept to pick monthly vs yearly)
  document.getElementById('btnBuyPro')?.addEventListener('click', handleBuyPro);
