 ...
 *  4.  Global State
 *  5.  DOM Cache
 *  6.  WPM-Based Level System (3 WPM = 1 level, purely performance)
...
const MAX_LEVEL   = 100;
const WPM_PER_LVL = 3;    // 3 WPM = 1 level
...
const S = {
  ...
  customCount: 50, customText: '', showAds: true,
  // Ghost
  ghostEnabled: false,
...
function cacheDOM() {
  const ids = [
    ...
    'lvlOrb','lvlNum','lvlDisp','lvlWpmLabel','lvlFill','lvlSub',
    ...
    'optAds','optPunct','optNums','fbCfgInput','btnConnFB','fbStatus','btnResetAll',
    ...
    'achToast','atIcon','atName','atXp',
    'toast','backdrop','themeToggle','adTop','adBot',
    'ghostToggleWrap','ghostToggle',
    'btnDaily','btnGhost','btnAch','btnLB','btnStats','btnSet','btnFriends',
    'ftStats','ftLB','ftPro','ftAch','ftDaily','ftFeedback',
  ];
...
function generateText() {
  const { mode, customCount, customText, usePunct, useNums, adaptiveOverride, diff } = S;
  const eff = adaptiveOverride || diff;

  if (mode === 'custom' && customText.trim()) return customText.trim();

  if (mode === 'punctuation') {
    const lines = shuffle([...PUNCT_LINES]);
    return lines.slice(0, Math.max(3, Math.ceil(customCount / 6))).join(' ');
  }

  if (mode === 'numbers') {
    const pool = WORDS.medium;
    const out  = [];
    const n    = Math.max(customCount, 20);
    for (let i = 0; i < n; i++) {
      if (Math.random() < 0.35) {
  ...
  if (mode === 'sentences') {
    ...
    return shuffle([...pool]).slice(0, 3).join(' ');
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (usePunct && i > 0 && Math.random() < 0.18) {
      words[words.length - 1] += PUNCTUATION[Math.floor(Math.random() * 4)];
    }
    words.push(w);
  }
  return words.join(' ');
...
function positionLiveCursor() {
  ...
  const pL = parseFloat(getComputedStyle(wrap).paddingLeft) || 24;
  const pT = parseFloat(getComputedStyle(wrap).paddingTop)  || 24;

  let left = eR.left - wR.left;
  // If cursor is past last typed char, move to right edge of previous char
  ...
  cursor.style.left   = `${left + pL}px`;
  cursor.style.top    = `${eR.top - wR.top + pT}px`;
...
function positionGhostCursor(charIdx) {
  ...
  const eR = tgEl.getBoundingClientRect();
  const pL = parseFloat(getComputedStyle(D.wordsWrap).paddingLeft) || 24;
  const pT = parseFloat(getComputedStyle(D.wordsWrap).paddingTop)  || 24;

  gc.style.left   = `${eR.left - wR.left + pL}px`;
  gc.style.top    = `${eR.top  - wR.top  + pT}px`;
...
const AC = {
  ts: [],
  record(t) { this.ts.push(t); },
  reset()   { this.ts = []; },
  check(wpm, acc) {
    if (wpm > 260) return { ok: false, why: 'WPM exceeds physical limit (260)' };
    const burst = this.ts.filter(t => t > performance.now() - 200).length;
    if (burst > 30) return { ok: false, why: 'Input burst detected' };
    if (wpm > 220 && acc === 100 && S.duration < 30) return { ok: false, why: 'Suspicious score' };
    return { ok: true };
  },
};
...
function initTest(isDaily = false) {
  ...
  if (window.innerWidth > 768) D.ghostInput?.focus();
}
...
function togglePause() {
  ...
  if (S.paused) {
    ...
    S.startTime += performance.now() - S.pauseStart;
...
function setFBStatus(msg, ok) {
  ...
  if (D.lbNote)   D.lbNote.textContent = ok && S.currentUser ? '' : ok ? 'Sign in to post globally.' : 'Firebase not connected — configure in Settings.';
}
...
async function renderLB(tab) {
  ...
  if (!S.fbReady) {
    D.lbBody.innerHTML = '<div class="lb-empty">Firebase not connected. Configure in Settings.</div>';
    return;
  }
...
function updateAuthUI() {
  ...
  if (D.authBtnTxt) D.authBtnTxt.textContent = u ? (u.displayName || u.email?.split('@')[0] || 'Me') : 'Sign In';
  if (D.authBtn)    D.authBtn.classList.toggle('in', !!u);
  if (D.lbNote)     D.lbNote.textContent = S.fbReady && u ? '' : S.fbReady ? 'Sign in to post globally.' : 'Firebase not connected.';
}
...
const ACH_DEFS = [
  // Speed milestones
  { id:'wpm_30',    ico:'🐢', name:'Turtle',          desc:'Reach 30 WPM',                              xp:50,   tier:'common', check: r => r.wpm >= 30 },
  { id:'wpm_50',    ico:'⚡', name:'Speed Demon',      desc:'Reach 50 WPM',                              xp:100,  tier:'common', check: r => r.wpm >= 50 },
  { id:'wpm_75',    ico:'🏃', name:'Fast Mover',       desc:'Reach 75 WPM',                              xp:175,  tier:'common', check: r => r.wpm >= 75 },
  { id:'wpm_100',   ico:'🔥', name:'Century',          desc:'Reach 100 WPM',                             xp:300,  tier:'rare',   check: r => r.wpm >= 100 },
  { id:'wpm_125',   ico:'💨', name:'Gale Force',       desc:'Reach 125 WPM',                             xp:500,  tier:'rare',   check: r => r.wpm >= 125 },
  { id:'wpm_150',   ico:'🌪', name:'Lightning',        desc:'Reach 150 WPM',                             xp:750,  tier:'rare',   check: r => r.wpm >= 150 },
  { id:'wpm_175',   ico:'☄', name:'Comet',             desc:'Reach 175 WPM',                             xp:1100, tier:'epic',   check: r => r.wpm >= 175 },
  { id:'wpm_200',   ico:'🚀', name:'Supersonic',       desc:'Reach 200 WPM',                             xp:1500, tier:'epic',   check: r => r.wpm >= 200 },
  { id:'wpm_220',   ico:'🛸', name:'Transcendent',     desc:'Reach 220 WPM',                             xp:2500, tier:'epic',   check: r => r.wpm >= 220 },
  // Accuracy
  { id:'acc_99',    ico:'✨', name:'Nearly Perfect',   desc:'99%+ accuracy on any test',                 xp:150,  tier:'common', check: r => r.acc >= 99 },
  { id:'acc_100',   ico:'💎', name:'Perfectionist',    desc:'100% accuracy on any test',                 xp:200,  tier:'rare',   check: r => r.acc === 100 },
  { id:'acc_100h',  ico:'🏆', name:'Flawless',         desc:'100% accuracy on Hard difficulty',          xp:500,  tier:'epic',   check: r => r.acc === 100 && r.diff === 'hard' },
  // Streaks
  { id:'str_25',    ico:'🔗', name:'Chain',            desc:'25+ character streak',                      xp:75,   tier:'common', check: r => r.streak >= 25 },
  { id:'str_50',    ico:'⛓', name:'Unbreakable',      desc:'50+ character streak',                      xp:150,  tier:'common', check: r => r.streak >= 50 },
  { id:'str_100',   ico:'🧲', name:'Magnetic',         desc:'100+ character streak',                     xp:300,  tier:'rare',   check: r => r.streak >= 100 },
  { id:'str_200',   ico:'∞',  name:'Infinite',         desc:'200+ character streak',                     xp:750,  tier:'epic',   check: r => r.streak >= 200 },
  // Test counts
  { id:'t1',        ico:'🎯', name:'First Blood',      desc:'Complete your first test',                  xp:25,   tier:'common', check: (_, h) => h.length >= 1 },
  { id:'t10',       ico:'📝', name:'Regular',          desc:'Complete 10 tests',                         xp:75,   tier:'common', check: (_, h) => h.length >= 10 },
  { id:'t50',       ico:'🏋', name:'Dedicated',        desc:'Complete 50 tests',                         xp:200,  tier:'common', check: (_, h) => h.length >= 50 },
  { id:'t100',      ico:'🎖', name:'Veteran',          desc:'Complete 100 tests',                        xp:500,  tier:'rare',   check: (_, h) => h.length >= 100 },
  { id:'t500',      ico:'🌟', name:'Legend',           desc:'Complete 500 tests',                        xp:2000, tier:'epic',   check: (_, h) => h.length >= 500 },
  // Levels
  { id:'lvl5',      ico:'🎮', name:'Getting Started',  desc:'Reach level 5',                             xp:50,   tier:'common', check: () => currentLevel() >= 5 },
  { id:'lvl10',     ico:'🥉', name:'Bronze Typist',    desc:'Reach level 10',                            xp:150,  tier:'common', check: () => currentLevel() >= 10 },
  { id:'lvl25',     ico:'🥈', name:'Silver Typist',    desc:'Reach level 25',                            xp:400,  tier:'rare',   check: () => currentLevel() >= 25 },
  { id:'lvl50',     ico:'🥇', name:'Gold Typist',      desc:'Reach level 50',                            xp:1000, tier:'rare',   check: () => currentLevel() >= 50 },
  { id:'lvl100',    ico:'👑', name:'Grand Master',     desc:'Reach level 100!',                          xp:5000, tier:'epic',   check: () => currentLevel() >= 100 },
  // Modes & special
  { id:'code',      ico:'🐒', name:'Code Monkey',      desc:'Complete a Code test',                      xp:100,  tier:'common', check: r => r.mode === 'code' },
  { id:'punct',     ico:'❗', name:'Punctuator',       desc:'Complete Punctuation mode',                 xp:100,  tier:'common', check: r => r.mode === 'punctuation' },
  { id:'nums',      ico:'🔢', name:'Numerist',         desc:'Complete Numbers mode',                     xp:100,  tier:'common', check: r => r.mode === 'numbers' },
  { id:'marathon',  ico:'🏃', name:'Marathon',         desc:'Complete a 120s test',                      xp:300,  tier:'rare',   check: r => r.duration >= 120 },
  { id:'hard',      ico:'💀', name:'Hard Mode',        desc:'Complete a Hard difficulty test',            xp:150,  tier:'common', check: r => r.diff === 'hard' },
  { id:'no_err',    ico:'🎯', name:'Zero Errors',      desc:'Finish any test with 0 errors',             xp:200,  tier:'rare',   check: r => r.errors === 0 },
  { id:'adaptive',  ico:'🧠', name:'Self-Aware',       desc:'Complete a test in Adaptive mode',          xp:120,  tier:'common', check: r => r._adaptive === true },
  // Ghost & Daily
  { id:'ghost_beat',ico:'👻', name:'Ghost Slayer',     desc:'Beat your own ghost record',                xp:250,  tier:'rare',   check: r => r._ghost_beat === true },
  { id:'daily1',    ico:'📅', name:'Daily Player',     desc:'Complete a daily challenge',                xp:150,  tier:'common', check: r => r._daily === true },
  { id:'daily7',    ico:'🗓', name:'Week Warrior',     desc:'Complete 7 daily challenges in a row',      xp:500,  tier:'rare',   check: () => getDailyStreak() >= 7 },
];
...
function showAchToast(ach) {
  ...
  if (D.atXp)   D.atXp.textContent   = `+${ach.xp} XP`;
...
═══════════════════════════════════════════════════════════ */
const DAILY_POOL = [
  { title:'Speed Sprint',       desc:'Hit 80+ WPM on a 30s Words/Medium test',         xp:500,  check: r => r.wpm >= 80  && r.mode === 'words'       && r.duration === 30 },
  { title:'Marathon Man',       desc:'Complete a 120s test with 90%+ accuracy',         xp:600,  check: r => r.acc >= 90  && r.duration >= 120 },
  { title:'Error-Free Zone',    desc:'Finish any 30s+ test with zero errors',           xp:400,  check: r => r.errors === 0 && r.duration >= 30 },
  { title:'Code Warrior',       desc:'Complete a Code test with 85%+ accuracy',         xp:450,  check: r => r.mode === 'code' && r.acc >= 85 },
  { title:'Hard Knocks',        desc:'Reach 60 WPM on Hard difficulty',                 xp:550,  check: r => r.diff === 'hard' && r.wpm >= 60 },
  { title:'Sentence Master',    desc:'Complete a Sentences test with 95%+ accuracy',    xp:500,  check: r => r.mode === 'sentences' && r.acc >= 95 },
  { title:'Streak Champion',    desc:'Achieve a 100+ character streak in one test',     xp:400,  check: r => r.streak >= 100 },
  { title:'Punctuation Pro',    desc:'Complete Punctuation mode with 90%+ accuracy',    xp:400,  check: r => r.mode === 'punctuation' && r.acc >= 90 },
  { title:'Number Cruncher',    desc:'Complete Numbers mode with 80%+ accuracy',        xp:350,  check: r => r.mode === 'numbers' && r.acc >= 80 },
  { title:'The Centurion',      desc:'Break 100 WPM in any test',                       xp:700,  check: r => r.wpm >= 100 },
  { title:'Consistency King',   desc:'Score 90%+ consistency on a 60s+ test',           xp:500,  check: r => r.cons >= 90 && r.duration >= 60 },
  { title:'Quick Draw',         desc:'Type 50+ WPM on a 15s test',                      xp:300,  check: r => r.wpm >= 50 && r.duration === 15 },
  { title:'Adaptive Master',    desc:'Complete any test in Adaptive difficulty mode',    xp:350,  check: r => r._adaptive === true },
  { title:'The Perfectionist',  desc:'100% accuracy on Hard difficulty',                xp:1000, check: r => r.acc === 100 && r.diff === 'hard' },
];
...
function initDailyChallenge() {
  ...
  if (!dc.done) {
    ...
    if (D.dbReward)    D.dbReward.textContent = `+${dc.xp} XP`;
...
function renderDailyModal() {
  ...
  D.dailyBody.innerHTML = `
    <div class="dc-card">
      ...
      <div class="dc-reward">🎁 Bonus: +${dc.xp} XP</div>
...
function checkDailyCompletion(result) {
  ...
  showToast(`🎉 Daily complete! +${dc.xp} XP bonus · ${streak}-day streak!`);
...
function buildShareText(r) {
  ...
  return `I just typed ${r.wpm} WPM with ${r.acc}% accuracy on TalionType!\n`
       ...
       + `Challenge me → ${window.location.href}`;
}
...
function loadPro() {
  ...
  if (S.isPro && D.btnPro) {
    D.btnPro.classList.add('active');
    D.btnPro.title = 'Pro unlocked ✓';
  }
  if (D.ghostToggleWrap) D.ghostToggleWrap.style.opacity = S.isPro ? '1' : '0.5';
  // Hide ads for Pro users on load
  if (D.adTop) D.adTop.hidden = S.isPro;
  if (D.adBot) D.adBot.hidden = S.isPro;
}
...
function verifyProKey() {
  ...
  if (valid) {
    ...
    if (D.adTop)           D.adTop.hidden = true;
    if (D.adBot)           D.adBot.hidden = true;
...
function readSettings() {
  ...
  S.showAds      = D.optAds?.checked     ?? true;
  applySettings();
}
...
function applySettings() {
  if (D.wordsWrap) D.wordsWrap.style.fontSize = S.fontSize + 'px';
  if (D.arena)     D.arena.classList.toggle('smooth-caret', S.smoothCaret);
  // Ads visible for free users, hidden for Pro
  if (D.adTop)     D.adTop.hidden = S.isPro;
  if (D.adBot)     D.adBot.hidden = S.isPro;
  if (D.ghostToggleWrap) D.ghostToggleWrap.style.opacity = S.isPro ? '1' : '0.5';
  localStorage.setItem(LS_SET, JSON.stringify({
    ...
    customCount: S.customCount, showAds: S.showAds,
  }));
}
...
function loadSettings() {
  try {
    ...
    ap('showAds',     D.optAds,     'showAds',      true);
  ...
  try {
    ...
    if (fb && D.fbCfgInput) {
      D.fbCfgInput.value = fb;
      initFirebase(JSON.parse(fb));
    }
  } catch {}
}
...
function loadTheme() {
  ...
  const c = localStorage.getItem('tt_color') || 'teal';
  document.documentElement.setAttribute('data-color', c);
  // Sync optDark checkbox if settings panel exists
  const optDark = document.getElementById('optDark');
  if (optDark) optDark.checked = (t === 'dark');
...
function openSettings()  { D.settingsPanel?.classList.add('open');    D.backdrop?.classList.add('active'); }
...
function openSaveModal() {
  ...
  if (D.saveNote)  D.saveNote.textContent = S.fbReady
    ...
    : 'Firebase not connected — saving locally only.';
...
function bindEvents() {
  ...
  // Friends (Pro only)
  D.btnFriends?.addEventListener('click', () => {
    if (!S.isPro) { renderProModal(); openModal(D.proOv); return; }
    openModal(D.friendsOv);
    const fb = document.getElementById('friendsBody');
    if (fb) fb.innerHTML = '<p style="color:var(--text-m);text-align:center;padding:2rem">Friend challenges coming soon — stay tuned!</p>';
  });
  document.getElementById('closeFriends')?.addEventListener('click', () => closeModal(D.friendsOv));
  ...
  [D.optSmooth, D.optSound, D.optLiveWpm, D.optAds, D.optPunct, D.optNums]
    .forEach(el => el?.addEventListener('change', readSettings));
  D.wordCount?.addEventListener('change', readSettings);
  ...
  document.getElementById('optDark')?.addEventListener('change', e => {
    ...
    localStorage.setItem('tt_theme', e.target.checked ? 'dark' : 'light');
  });
  ...
  D.ftFeedback?.addEventListener('click', e => {
    e.preventDefault();
    if (!S.isPro) { renderProModal(); openModal(D.proOv); return; }
    openModal(document.getElementById('feedbackOv'));
  });
  document.getElementById('closeFeedback')?.addEventListener('click', () => closeModal(document.getElementById('feedbackOv')));
  document.getElementById('btnSendFeedback')?.addEventListener('click', () => {
    ...
    window.location.href = `mailto:hello@talionlabs.com?subject=TalionType%20Feedback&body=${body}`;
  });

  // Color picker (Pro only)
  document.querySelectorAll('.cpick').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      localStorage.setItem('tt_color', color);
    });
  });

  D.btnConnFB?.addEventListener('click', () => {
    try { initFirebase(JSON.parse(D.fbCfgInput?.value.trim() || '{}')); }
    catch { setFBStatus('Invalid JSON — check your Firebase config.', false); }
  });
