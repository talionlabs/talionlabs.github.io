 ...
 * ════════════════════════════════════════════════════════════════
 * A TalionType product · https://taliontype.pages.dev
 *
 * SECTIONS:
 *  1.  Config & Constants
 *  2.  [Firebase — configured via initFirebase() call in §36]
 ...
 *  29. Achievement System — 95+ Achievements (Common / Rare / Epic / Legendary)
 *  30. Daily Challenge
 *  31. Social Sharing
 *  32. Sound Engine
 *  33. Pro / Gumroad
...
═══════════════════════════════════════════════════════════ */
const VER         = '3.0.0';
const HOME_URL    = 'https://taliontype.pages.dev';
const GUMROAD_URL = 'https://gumroad.com/l/taliontype-pro'; // update this
const MAX_LEVEL   = 100;
// FIX: non-linear level thresholds — all whole numbers, max 320 WPM at level 100
// Levels 1→50:  2 WPM per level (easy start, covers 0–100 WPM)
// Levels 51→80: 4 WPM per level (mid-range, covers 100–220 WPM)
// Levels 81→100: 5 WPM per level (elite range, covers 220–320 WPM)
const LEVEL_THRESHOLDS = (() => {
  const t = [0]; // index 0 unused; t[1]=0 means level 1 starts at 0 WPM
  t[1] = 0;
  for (let i = 2; i <= 50;  i++) t[i] = t[i-1] + 2;
  for (let i = 51; i <= 80;  i++) t[i] = t[i-1] + 4;
  for (let i = 81; i <= 100; i++) t[i] = t[i-1] + 5;
  return t; // t[100] === 320
})();
...
const WORDS = {
  easy: [
    ...
    'some','could','them','see','than','then','now','look',
    'only','come','its','over','think','also','back','after',
    'two','how','our','work','well','way','even','new',
    'want','any','give','day','most','us','too','big',
    'man','old','ask','part','run','try','much','let',
    'put','end','why','long','home','hand','place','case',
    'week','fact','group','point','play','call','off','need',
    'same','tell','boy','help','live','hold','word','town',
    'find','next','keep','three','turn','four','five','six',
    'seven','eight','nine','ten','last','left','right','side',
    'high','move','feel','state','set','done','said','here',
    'ago','far','own','few','life','open','seem','form',
    'door','cut','show','watch','stop','air','land','sea',
    'fire','tree','name','read','book','page','walk','road',
    'car','bus','hot','cold','win','lost','idea','game',
    'line','step','face','small','food','room','real','sun',
    'moon','night','yes','ok','hear','love','hate','fear',
    'kind','nice','bad','best','free','true','fast','slow',
    'hard','soft','dark','light','low','down','front','plan',
    'yet','both','once','area','half','white','black','blue',
    'red','green','top','base','body','arm','leg','eye',
    'ear','mouth','nose','hair','foot','head','dog','cat',
    'bird','fish','cow','pig','hat','cup','box','bag',
    'key','pen','bed','sit','stand','jump','cry','eat',
    'drink','sleep','wake','talk','close','push','pull','bring',
    'send','buy','sell','pay','cost','save','lose','fall',
    'grow','break','fix','build','use','add','change','hope',
    'wish','wait','rest','stay','age','art','fog','fun',
    'gap','gas','god','gun','ice','job','joy','kid',
    'law','lot','map','mix','net','nor','oak','oil',
    'pit','pop','pot','raw','ray','row','rub','shy',
    'sky','sow','spa','spy','sum','tap','tax','tea',
    'tie','tip','toe','toy','tub','war','wit','woe',
    'zoo','ache','acre','acid','aide','aims','aisle','ally',
    'arch','arms','atom','axle','baby','bail','bake','bald',
    'ball','band','bang','bank','bare','barn','bath','beam',
    'bean','bear','beat','beef','bell','belt','bend','bite',
    'blow','blur','bolt','bond','bone','boom','boot','bore',
    'born','boss','bowl','brow','bulk','bull','burn','busy',
    'cage','calm','camp','card','care','cash','cast','cave',
    'cell','chin','chip','chop','clay','clip','club','coal',
    'coat','code','coil','coin','core','cork','corn','couch',
    'coup','cozy','crab','crew','crop','curl','cute','deck',
    'deed','deem','deer','dent','desk','dice','dime','dish',
    'disk','dock','doll','dome','doom','dose','dram','draw',
    'drew','drip','drop','drum','dual','duel','dusk','dust',
    'earl','earn','ease','echo','edit','else','envy','epic',
    'exam','fake','fame','farm','fate','felt','file','fill',
    'film','firm','fist','flag','flat','flew','flit','flow',
    'foam','folk','fond','fool','fort','foul','fowl','fume',
    'fund','fury','fuse','gale','gang','gate','gave','gaze',
    'gear','germ','gift','gist','glad','glow','glue','goal',
    'gold','gone','gore','gown','grab','grip','gust','hack',
    'hall','halt','harm','haze','heal','heap','heat','heel',
    'heir','helm','herb','herd','hero','hint','hire','holy',
    'hook','horn','host','hour','hull','hung','hunt','hurt',
    'hymn','icon','idle','iris','isle','item','jade','jail',
    'jest','join','joke','jolt','jury','keen','kill','king',
    'knee','knit','knob','lack','laid','lame','lamp','lane',
    'lead','leaf','lean','leap','lend','lens','liar','lift',
    'lime','link','list','load','loan','lock','loft','logo',
    'lone','lore','lump','lung','lure','lurk','lust','mail',
    'main','male','mall','mane','mare','mark','mars','mass',
    'mast','melt','mesh','mild','mill','mint','miss','mist',
    'moat','mock','mole','moor','more','moth','muck','muse',
    'myth','nail','near','neat','neck','norm','note','nous',
    'oath','obey','odds','omen','oval','oven','pace','pack',
    'pair','pale','palm','park','pass','past','path','pave',
    'pawn','peak','peel','peer','pick','pile','pine','pipe',
    'pity','plot','ploy','plug','plum','plus','poem','poet',
    'pole','poll','pond','pore','pose','pour','prey','prim',
    'prod','prop','prow','pump','pure','quit','race','rack',
    'rage','raid','rail','rake','ramp','rank','rant','rate',
    'reap','reel','rely','rent','rice','rich','ride','rift',
    'rime','riot','rise','risk','rite','robe','rock','role',
    'roll','roof','root','rope','rose','rout','rude','ruin',
    'rule','rush','rust','safe','saga','sail','sake','salt',
    'sang','sank','sash','seal','seam','seek','self','sent',
    'shed','shin','shot','silk','sill','sink','slab','slag',
    'slam','slap','slim','slip','slot','slug','smug','snap',
    'snow','soap','sock','soda','soil','sold','sole','song',
    'soot','sore','sort','soup','sour','span','spin','spit',
    'spot','spur','stab','stag','star','stem','stew','stir',
    'stub','stun','such','suit','sung','sunk','swap','swat',
    'sway','swum','tail','tale','tall','tame','taps','task',
    'team','tear','teen','term','test','text','thin','thorn',
    'tide','till','told','toll','tomb','tone','took','torn',
    'toss','tour','trek','trim','trio','trip','troy','tuck',
    'tuft','twig','twin','type','urge','vale','vane','vary',
    'vast','vein','vest','view','vine','vise','void','volt',
    'vote','wage','wane','warp','wart','wasp','wave','weak',
    'weal','weld','went','were','west','whim','whip','wilt',
    'wind','wine','wing','wire','wise','wits','wolf','womb',
    'wood','wool','wore','worm','wove','wrap','wren','writ',
    'yell','yore','zinc','zone',
  ],
  medium: [
    'ability','absence','account','achieve','acquire','address','advance','airline',
    'ancient','another','anxiety','approve','archive','arrange','article','attempt',
    'average','balance','battery','between','billion','brought','cabinet','capital',
    'captain','careful','central','certain','chapter','charity','climate','college',
    'combine','command','comment','complex','concern','conduct','confirm','connect',
    'context','control','correct','council','country','culture','current','decided',
    'declare','defined','deliver','develop','digital','display','diverse','driving',
    'dynamic','economy','element','embrace','emotion','endless','enhance','explore',
    'factory','feeling','finance','forward','freedom','further','genuine','history',
    'however','imagine','include','involve','justice','kitchen','language','leading',
    'library','limited','message','mistake','morning','natural','nothing','obvious',
    'opinion','outside','patient','payment','perfect','picture','problem','process',
    'product','protect','purpose','quickly','realize','receive','related','replace',
    'respect','results','science','service','several','similar','society','student',
    'subject','suggest','support','teacher','thought','through','tonight','trouble',
    'usually','various','version','village','website','whether','already','because',
    'business','change','coming','company','abstract','academy','accident','accurate',
    'action','active','actual','adapt','adjust','admire','affect','afford',
    'agency','agenda','agree','allocate','allow','alter','analysis','announce',
    'answer','appeal','apply','arrive','aspect','assign','assist','assume',
    'attract','barrier','battle','behave','belief','benefit','beyond','boundary',
    'branch','bridge','budget','capable','capture','career','catalog','category',
    'challenge','channel','choice','citizen','clarity','classic','collect','column',
    'compete','compile','component','concept','condition','conflict','consider','consume',
    'contain','contribute','convert','create','criteria','cycle','danger','debate',
    'decade','decide','decline','decrease','default','define','degree','demand',
    'design','despite','detect','device','differ','direct','discover','discuss',
    'distance','distinct','divide','domain','double','draft','effort','either',
    'energy','engage','entire','equal','establish','evaluate','evidence','example',
    'expand','expect','experience','explain','express','extend','extreme','factor',
    'failure','feature','focus','follow','format','foundation','future','general',
    'generate','global','growth','handle','happen','highest','hundred','impact',
    'improve','increase','indicate','industry','inform','initial','inspire','install',
    'instead','interest','invest','isolate','iterate','journey','justify','knowledge',
    'launch','layout','leader','lesson','listen','logical','maintain','manage',
    'matter','measure','method','mission','modify','monitor','narrow','network',
    'normal','notice','number','object','observe','obtain','office','operate',
    'option','organize','outcome','output','overcome','overview','parent','partner',
    'pattern','people','perform','period','permit','place','planning','platform',
    'policy','popular','position','power','predict','prefer','prepare','present',
    'prevent','primary','principle','prior','profit','project','promote','provide',
    'publish','quality','question','range','rating','reason','reduce','reflect',
    'regard','region','remain','remove','repeat','require','research','resolve',
    'resource','respond','restore','result','return','review','reward','sample',
    'secure','select','series','setting','single','social','solve','source',
    'specific','status','strategy','stream','strong','structure','success','summary',
    'system','target','technical','theory','transfer','travel','trigger','typical',
    'update','useful','utilize','value','verify','visible','volume','warning',
    'welcome','within','without','wonder','accessible','according','acknowledge','absorb',
    'accent','accept','access','accrue','ache','acorn','across','adhere',
    'admit','adopt','adult','ahead','alarm','alert','align','alike',
    'allot','along','amend','ample','angle','annex','apart','arise',
    'aside','asset','attic','audio','audit','avid','award','avoid',
    'basic','basis','batch','begin','blend','block','bloom','bonus',
    'boost','bound','brain','brand','brave','brick','brief','bring',
    'broad','brush','cabin','cache','check','child','civic','civil',
    'claim','class','clerk','click','close','coach','color','comic',
    'craft','crash','cross','crowd','daily','dance','datum','depth',
    'diary','dirty','drain','dream','dress','drive','eagle','early',
    'earth','elect','elite','email','embed','empty','enter','entry',
    'exact','exist','extra','faith','false','fancy','field','final',
    'first','fixed','flame','floor','flora','fluid','flush','focal',
    'force','forge','found','frame','frank','front','gauge','given',
    'grade','greet','group','guard','guide','guild','habit','happy',
    'haven','heavy','honor','house','human','humid','ideal','image',
    'imply','inner','input','inter','intro','issue','joint','judge',
    'junior','label','large','later','layer','learn','legal','level',
    'light','limit','liner','local','loose','lower','lucky','lunar',
    'magic','major','maker','match','media','micro','miles','minor',
    'model','moral','mount','movie','music','never','night','noble',
    'north','novel','nurse','offer','often','onset','order','other',
    'outer','owner','pages','panel','paper','peace','phase','phone',
    'pilot','plain','plant','plate','plaza','plead','plenty','point',
    'polar','probe','proof','prose','prove','proxy','pulse','radar',
    'raise','rapid','ratio','reach','ready','realm','relay','reply',
    'reset','right','robot','rocky','rough','round','royal','rural',
    'scale','scene','scope','score','scout','sense','serve','share',
    'sharp','shift','short','sight','skill','smart','smile','solid',
    'speed','spend','stage','stand','state','steel','steep','stick',
    'still','stock','store','storm','story','study','style','sugar',
    'suite','super','sweet','sword','table','taste','teach','terms',
    'thick','title','token','topic','touch','tough','track','trade',
    'train','trait','treat','trend','trial','ultra','under','union',
    'unity','urban','usage','users','valid','venue','verse','video',
    'viral','vista','vital','voice','voter','waste','water','whole',
    'wider','winter','world','worth','young','youth',
  ],
  hard: [
    'aberration','abominable','accelerate','accommodate','acknowledge','acquisition','ambiguous','anachronism',
    'anticipate','apocalyptic','apparatus','architecture','arithmetic','assassination','astonishment','atmosphere',
    'authoritative','bureaucracy','catastrophe','circumstances','clarification','collaboration','complicated','comprehensive',
    'concentration','configuration','consciousness','contradiction','controversial','coordination','cryptocurrency','deterioration',
    'determination','disambiguation','diversification','electromagnetic','establishment','exacerbation','extrapolate','fluorescent',
    'hallucination','heterogeneous','hierarchical','hypothetical','identification','implementation','incompatible','infrastructure',
    'instantiation','juxtaposition','lexicographical','manifestation','metamorphosis','miscommunication','multidimensional','obfuscation',
    'orchestration','overwhelming','perpendicular','philanthropist','physiological','predetermined','prioritization','quintessential',
    'rationalization','reconnaissance','reverberation','revolutionary','sophisticated','straightforward','subconscious','transcontinental',
    'unintelligible','unprecedented','vulnerabilities','entrepreneurship','acknowledgement','categorization','contemporaneous','disproportionate',
    'extraordinarily','incomprehensible','indispensable','abnormality','abstraction','accumulation','acquaintance','administration',
    'advantageous','affiliation','aggravation','ameliorate','amplification','approximation','articulation','authentication',
    'automation','biological','capitalization','certification','characterized','chronological','classification','communication',
    'compensation','compilation','composition','computational','conventional','crystallization','customization','decomposition',
    'deliberation','demonstration','denomination','deportation','depreciation','differentiation','dimensionality','disorientation',
    'dissatisfaction','documentation','dramatization','dysfunction','ecological','elaboration','elimination','emancipation',
    'encouragement','enlightenment','enumeration','environment','equalization','estimation','evaluation','examination',
    'expropriation','extraordinary','falsification','familiarization','fascination','formalization','fragmentation','fundamentalism',
    'globalization','gravitational','harmonization','immunological','implication','incarceration','inconsistency','independence',
    'indeterminate','industrialization','inflammation','initialization','interpretation','intimidation','investigation','liberalization',
    'linearization','manipulation','marginalization','maximization','memorization','miscalculation','modernization','multiplication',
    'normalization','objectification','optimization','organizational','parliamentary','participation','pathological','perfectionistic',
    'personalization','phenomenological','philosophical','pluralization','popularization','precipitation','preoccupation','probabilistic',
    'professionalism','proportionality','psychological','qualification','quantification','reconfiguration','reconstruction','redistribution',
    'refactoring','regularization','rehabilitation','reincarnation','reiteration','representation','reproduction','responsibility',
    'restructuring','semiconductor','sensationalism','simplification','socialization','spiritualization','standardization','stereotyping',
    'stratification','synchronization','systematization','technological','telecommunication','transformation','transparency','triangulation',
    'unambiguous','underestimated','unimaginable','universalization','vulnerability','abnormalities','acceleration','accessibility',
    'accountability','actualization','algorithmic','anachronistic','anthropological','asymptotic','bureaucratic','characterization',
    'codification','commercialization','commodification','complementary','decentralization','democratization','exaggeration','incompatibility',
  ],
};
...
function cacheDOM() {
  const ids = [
    ...
    'wordCount','customTxt','optCustomLoop','optSmooth','optSound','optLiveWpm',
    'optPunct','optNums','optProgress','optBlink','optErrSnd','optHiCon','optReducedMotion','btnResetAll','btnResetSettings',
...
═══════════════════════════════════════════════════════════
   Level = LEVEL_THRESHOLDS lookup, capped at MAX_LEVEL (100).
   Max WPM for level 100 = 320 WPM (whole-number steps: 2/4/5 WPM).
   Level ONLY advances if user beats their personal best WPM.
═══════════════════════════════════════════════════════════ */
function calcLevel(wpm) {
  if (wpm <= 0) return 1;
  for (let i = MAX_LEVEL; i >= 1; i--) {
    if (wpm >= LEVEL_THRESHOLDS[i]) return i;
  }
  return 1;
}
...
function refreshLevelUI() {
  ...
  const hi        = lvl >= MAX_LEVEL ? LEVEL_THRESHOLDS[MAX_LEVEL] : LEVEL_THRESHOLDS[lvl + 1];
...
function generateText() {
  ...
  if (mode === 'custom' && customText.trim()) {
    if (S.customLoop) {
      // Loop the custom text words to fill exactly customCount words
      const srcWords = customText.trim().split(/\s+/).filter(Boolean);
      if (!srcWords.length) return customText.trim();
      const target = Math.max(1, customCount);
      const out = [];
      for (let i = 0; out.length < target; i++) {
        out.push(srcWords[i % srcWords.length]);
      }
      return out.join(' ');
    }
  ...
  if (!S.isPro) {
    D.ghostBody.innerHTML = `
      <div style="text-align:center;padding:1.5rem">
        ...
        <button class="ctrl-btn accent" id="ghostProBtn" style="margin:0 auto">⚡ Upgrade to Pro</button>
      </div>`;
...
const AC = {
  ...
  check(wpm, acc) {
    ...
    if (times.length > 5) {
      ...
      // Minimum inter-keystroke check: humans cannot physically type faster than ~30ms apart
      let tooFast = 0;
      for (let i = 1; i < times.length; i++) {
        if (times[i] - times[i - 1] < 30) tooFast++;
      }
      // Allow a few accidental near-simultaneous presses (e.g. rolling technique)
      if (tooFast > times.length * 0.15) return { ok: false, why: 'Inhuman keystroke timing detected' };
    }
...
function finishTest() {
  ...
  // Build result
  const result = {
    wpm, rawWpm: raw, acc, consistency: cons, errors: errs,
    ...
    _ghost_beat: S.ghostEnabled && S.isPro && !!S.ghostRecord && wpm > (S.ghostCustomWpm ?? S.ghostRecord?.wpm ?? 0),
  };
  S.lastResult = result;
  saveHistory(result);
  persistProfile();
...
function initTest(isDaily = false) {
  ...
  // D: If custom mode but no text saved yet, show a helpful placeholder instead of the arena
  if (S.mode === 'custom' && !S.customText.trim()) {
    if (D.wordsTarget) D.wordsTarget.innerHTML = '';
    if (D.wordsTyped)  D.wordsTyped.innerHTML  = '';
    if (D.arenaHint) {
      D.arenaHint.style.opacity = '1';
      D.arenaHint.innerHTML = '⚙ Enter your custom text in <b>Settings</b> to begin';
    }
    if (D.ghostInput) D.ghostInput.disabled = true;
    updateModeFlags();
    return; // Don't render arena — no text to show
  }

  // Restore default arena hint for all other modes
  if (D.arenaHint) D.arenaHint.innerHTML = 'Click here or press <kbd>Tab</kbd> to begin typing';
...
// LB cache — reduces Firestore reads by ~90% on repeat views
const _lbCache = {};
const LB_CACHE_TTL = 5 * 60 * 1000; // 5 minutes
function getCachedLB(key) {
  const c = _lbCache[key];
  if (c && Date.now() - c.ts < LB_CACHE_TTL) return c.data;
  return null;
}
function setCachedLB(key, data) { _lbCache[key] = { data, ts: Date.now() }; }
function bustLBCache() { Object.keys(_lbCache).forEach(k => delete _lbCache[k]); }
...
function initFirebase(cfg) {
  try {
    ...
    // A: Activate App Check (reCAPTCHA v3) — requires the app-check-compat script in index.html
    // IMPORTANT: If users see "The requested action is invalid" when signing in, go to
    // Firebase Console → App Check → set Firestore + Auth to "Monitor" (not "Enforce").
    // Only switch to Enforce once you've confirmed valid tokens appear in the monitor logs.
    try {
      if (typeof firebase.appCheck === 'function') {
        const appCheck = firebase.appCheck(S.fbApp);
        appCheck.activate(
          new firebase.appCheck.ReCaptchaV3Provider('6LeJhnosAAAAALRupckQsdkpsONV4zPzIkPliqFX'),
          true // auto-refresh token
        );
      }
    } catch (acErr) { console.warn('App Check skipped:', acErr.message); }

    S.fbDB   = firebase.firestore(S.fbApp);
    S.fbAuth = firebase.auth(S.fbApp);

    // C: When auth state resolves, sign in anonymously if no real account is present
    // so unauthenticated visitors can still read the leaderboard via Firestore rules.
    S.fbAuth.onAuthStateChanged(u => {
      S.currentUser = u;
      updateAuthUI();
      if (!u) {
        // No user at all — try anonymous auth so Firestore reads work for visitors
        S.fbAuth.signInAnonymously().catch(() => {});
      } else if (!u.isAnonymous) {
        // FIX: silently check for Pro custom claim every time a real user signs in
        checkProClaimOnSignIn(u);
      }
    });
...
async function saveGlobalScore(name, r) {
  // C: Block anonymous users — they can view leaderboard but not post to it
  if (!S.fbDB || !S.currentUser || S.currentUser.isAnonymous) return;
  ...
  bustLBCache(); // invalidate cache after a new score is posted
...
async function loadFirestoreLB(colName, filters = {}) {
  ...
  const cacheKey = colName + JSON.stringify(filters);
  ...
  try {
    ...
    const result = snap.docs.map((d, i) => ({
      rank:  i + 1,
      uid:   d.data().uid,
...
async function renderLB(tab) {
  ...
  // FIX: if Firebase is ready but user isn't signed in yet (anon sign-in still pending),
  // wait briefly for anonymous auth to complete so Firestore reads can proceed
  if (!S.currentUser && S.fbAuth) {
...
function updateAuthUI() {
  ...
  // C: Anonymous users (added for leaderboard reads) should not appear as "signed in"
  const isAnon  = u?.isAnonymous === true;
  const loggedIn = !!u && !isAnon;
  const label = loggedIn ? (u.displayName || u.email?.split('@')[0] || 'Me') : 'Sign In';
  if (D.authBtnTxt)   D.authBtnTxt.textContent   = label;
  if (D.mmAuthBtnTxt) D.mmAuthBtnTxt.textContent  = label;
  if (D.authBtn)    D.authBtn.classList.toggle('in', loggedIn);
  if (D.mmAuthBtn)  D.mmAuthBtn.classList.toggle('in', loggedIn);
  if (D.lbNote)     D.lbNote.textContent = S.fbReady && loggedIn ? '' : S.fbReady ? 'Sign in to post scores globally.' : 'Sign in to post to the global leaderboard.';
}
...
async function doGoogleSignIn() {
  ...
  try {
    ...
    // FIX: App Check enforcement can cause "The requested action is invalid".
    // If you see this error, go to Firebase Console → App Check and switch
    // Firestore + Auth from "Enforce" to "Monitor" mode temporarily.
    let msg = e.message;
    if (e.code === 'auth/unauthorized-domain') {
      msg = 'This domain is not authorized. Add it in Firebase Console → Authentication → Authorized Domains.';
    } else if (msg.includes('requested action is invalid') || e.code === 'auth/internal-error') {
      msg = 'Sign-in blocked — likely App Check enforcement. Go to Firebase Console → App Check → switch to Monitor mode, then try again.';
    }
    if (D.authErr) D.authErr.textContent = msg;
  }
}
...
const ACH_DEFS = [
  ...
  { id:'wpm_40',    ico:'🚶', name:'Casual Typist',      desc:'Reach 40 WPM',           tier:'common',    check: r => r.wpm >= 40 },
  ...
  { id:'lvl90',     ico:'🚀', name:'Grand Master',        desc:'Reach level 90',         tier:'epic',      check: () => currentLevel() >= 90 },
  ...
  // ── Fun / Easter eggs ────────────────────────────────────
  { id:'night_owl', ico:'🦉', name:'Night Owl',          desc:'Type after midnight',    tier:'common',    check: () => new Date().getHours() < 5 },
  ...
  { id:'rage_quit', ico:'🔄', name:'On a Streak',        desc:'Complete 5 tests in a row',tier:'common',  check: (_, h) => h.length >= 5 },
  ...
  if (!S.isPro) {
    container.innerHTML = `
      <div style="text-align:center;padding:1.5rem">
        ...
        <button class="ctrl-btn accent" id="chatProBtn">⚡ Upgrade to Pro</button>
      </div>`;
...
function getDailyDef() {
  ...
  const lvl     = currentLevel();
  const bestWpm = S.bestWpm || 0;

  // Filter by level range AND ensure WPM requirement doesn't exceed the user's current ability
  // Allow a small stretch (120% of best WPM) so the challenge is still motivating but achievable
  const eligible = DAILY_POOL.filter(c => {
    const withinLevel  = lvl >= c.minLevel && lvl <= c.maxLevel;
    const wpmReachable = !c.requiredWpm || bestWpm === 0 || bestWpm * 1.2 >= c.requiredWpm;
    return withinLevel && wpmReachable;
  });

  // Fallback: if no challenges match (brand-new user or edge case),
  // use the ones with the lowest minLevel to avoid impossible tasks
  const sorted = DAILY_POOL.slice().sort((a, b) => a.minLevel - b.minLevel);
  const fallback = sorted.filter(c => !c.requiredWpm || bestWpm === 0 || bestWpm * 1.2 >= c.requiredWpm);
  const pool  = eligible.length ? eligible : (fallback.length ? fallback.slice(0, 3) : [DAILY_POOL[0]]);
...
function estimatePercentile(wpm) {
  // Based on typing speed distribution benchmarks
  if (wpm >= 160) return 99;
  if (wpm >= 130) return 97;
...
function generateShareImage(r) {
  ...
  const C = name => getComputedStyle(document.documentElement).getPropertyValue(name).trim() || '#00D9B0';
  const acc = C('--acc') || '#00D9B0';
  const bg  = C('--bg-card') || '#0C1018';
  const tf  = C('--text') || '#E8ECF4';
  const tm  = C('--text-m') || '#8FA3BF';
  ...
  ctx.textAlign = 'left';
  ctx.fillText(r.wpm, 60, 200);

  ctx.fillStyle = tm;
  ctx.font = '32px "Outfit", sans-serif';
  ctx.fillText('WPM', 60, 240);

  // Stats
  ctx.fillStyle = tf;
  ctx.font = '28px "Outfit", sans-serif';
  const pct = estimatePercentile(r.wpm);
  ctx.fillText(`Faster than ${pct}% of typists`, 60, 300);
  ctx.fillStyle = tm;
  ctx.font = '22px "Outfit", sans-serif';
  ctx.fillText(`${r.acc}% accuracy  ·  ${r.errors} errors  ·  ${r.mode}/${r.diff}`, 60, 340);

  // Branding
  ctx.fillStyle = acc;
  ctx.font = 'bold 22px "Syne", sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText('TalionType', 760, 370);
  ctx.fillStyle = tm;
  ctx.font = '16px "Outfit", sans-serif';
  ctx.fillText('taliontype.pages.dev', 760, 392);
...
function openShareModal() {
  ...
  // Mark as shared so the Braggart achievement can unlock
  if (S.lastResult) { S.lastResult._shared = true; checkAchievements(S.lastResult); }
  const pct = estimatePercentile(r.wpm);
  if (D.shareCard) {
    D.shareCard.innerHTML = `
      <div style="font-size:2.5rem;font-weight:700;color:var(--acc)">${r.wpm} WPM</div>
      <div style="margin:.3rem 0;font-size:1.05rem;color:var(--text)">Faster than <b style="color:var(--acc)">${pct}%</b> of typists</div>
      <div style="margin:.4rem 0;color:var(--text-m)">${r.acc}% accuracy · ${r.errors} errors</div>
      <div style="font-size:.75rem;color:var(--text-f)">${r.mode} / ${r.diff} · ${r.duration}s · TalionType</div>`;
  }
  openModal(D.shareOv);
}
...
function playSound(type) {
  ...
  try {
    ...
    const vol = _audioVolume * (type === 'c' ? 0.028 : 0.035);
    if (type === 'c') { osc.type = 'sine';   osc.frequency.value = 1047; gain.gain.value = vol; }
    else              { osc.type = 'square'; osc.frequency.value = 196;  gain.gain.value = vol; }
    osc.start();
...
const PRO_FEATURES_DEF = [
  ...
  { ico:'📅', name:'Daily Challenges',  sub:'Level-matched challenges with streak tracking' },
...
// FIX: called after sign-in to silently check Firebase custom claims and grant Pro if found
async function checkProClaimOnSignIn(user) {
  if (!user || user.isAnonymous || S.isPro) return;
  try {
    const tokenResult = await user.getIdTokenResult(true);
    if (tokenResult.claims?.pro === true) {
      grantPro();
    }
  } catch {}
}
...
// FIX: Pro verification via Firebase custom claims (set by Cloudflare Worker webhook from Gumroad).
// When someone buys on Gumroad, the webhook calls Firebase Admin to set claims.pro = true on their account.
// We verify by refreshing the ID token and reading the claim — no client-side bypass possible.
async function verifyProClaim() {
  const u = S.currentUser;
  if (!u || u.isAnonymous) {
    if (D.proVerifyNote) {
      D.proVerifyNote.style.color = 'var(--err)';
      D.proVerifyNote.textContent = 'Sign in first, then click Verify after purchasing.';
    }
    return;
  }

  if (D.proVerifyNote) {
    D.proVerifyNote.style.color = 'var(--text-m)';
    D.proVerifyNote.textContent = 'Checking your account…';
  }

  try {
    // Force token refresh to get latest custom claims from server
    ...
    if (tokenResult.claims?.pro === true) {
      ...
      if (D.proVerifyNote) {
        D.proVerifyNote.style.color = 'var(--err)';
        D.proVerifyNote.textContent = 'No Pro access found. Purchase on Gumroad, then wait 1–2 minutes and try again.';
      }
    }
  } catch (e) {
    if (D.proVerifyNote) {
      D.proVerifyNote.style.color = 'var(--err)';
      D.proVerifyNote.textContent = 'Error: ' + e.message;
    }
  }
}
...
function readSettings() {
  ...
  S.customLoop   = D.optCustomLoop?.checked ?? false;
  S.customText   = D.customTxt?.value    || '';
  // E: These were wired to readSettings in bindEvents but never read — fixed
  S.noBlink       = !(D.optBlink?.checked          ?? true);  // FIX: checked=ON means blink IS active, so noBlink=false
  S.showProgress  = D.optProgress?.checked       ?? true;
  S.errSnd        = D.optErrSnd?.checked         ?? false;
  S.hiCon         = D.optHiCon?.checked          ?? false;
  S.reducedMotion = D.optReducedMotion?.checked  ?? false;

  // Word count: default = 750
...
function applySettings() {
  ...
  localStorage.setItem(LS_SET, JSON.stringify({
    ...
    customCount: S.customCount, customLoop: S.customLoop, noBlink: S.noBlink, showProgress: S.showProgress,
    errSnd: S.errSnd, hiCon: S.hiCon, reducedMotion: S.reducedMotion,
    customText: S.customText,
    // FIX: persist mode/diff/duration so they survive a reload
    mode: S.mode, diff: S.diff, duration: S.duration,
  }));
}
...
function loadSettings() {
  try {
    ...
    ap('customLoop',   D.optCustomLoop,     'customLoop',    true);
    ...
    // D: restore saved custom text into the textarea and S.customText
    ap('customText',   D.customTxt,         'customText');
    if (D.fsVal) D.fsVal.textContent = (S.fontSize || 20) + 'px';

    // FIX: restore mode / diff / duration so the user's last choice survives reload
    if (s['mode']     && ['words','sentences','code','punctuation','numbers','custom'].includes(s['mode'])) S.mode = s['mode'];
    if (s['diff']     && ['easy','medium','hard','adaptive'].includes(s['diff'])) S.diff = s['diff'];
    if (s['duration'] && [15, 30, 60, 120].includes(+s['duration'])) S.duration = +s['duration'];
  } catch {}
...
// FIX: lightweight save of mode/diff/duration so they survive reload
function saveModePrefs() {
  try {
    const s = JSON.parse(localStorage.getItem(LS_SET) || '{}');
    s.mode = S.mode; s.diff = S.diff; s.duration = S.duration;
...
function bindEvents() {
  ...
  D.modePills.forEach(p => p.addEventListener('click', () => {
    ...
    saveModePrefs(); // FIX: persist selection
    initTest();
  }));
  ...
  D.btnVerifyPro?.addEventListener('click', verifyProClaim);
  ...
  document.getElementById('btnDlImg')?.addEventListener('click', () => {
    const r = S.lastResult; if (!r) return;
    try {
      const dataUrl = generateShareImage(r);
      const a = document.createElement('a');
      a.href = dataUrl;
      a.download = `taliontype-${r.wpm}wpm.png`;
      a.click();
      gaEvent('share', { platform: 'image_download' });
    } catch (e) { showToast('Could not generate image in this browser'); }
  });

  // Settings controls
  // Dark mode checkbox — mirrors the theme toggle
  const optDark = document.getElementById('optDark');
  if (optDark) {
    // Sync initial state with current theme
    optDark.checked = document.documentElement.getAttribute('data-theme') !== 'light';
    optDark.addEventListener('change', () => {
      const next = optDark.checked ? 'dark' : 'light';
  ...
  // Volume slider (previously dead, now wired)
  const volSlider = document.getElementById('volSlider');
  const volVal    = document.getElementById('volVal');
  if (volSlider) {
    // Restore saved volume
    const savedVol = parseFloat(localStorage.getItem('tt_volume') ?? '0.6');
    _audioVolume   = savedVol;
    volSlider.value = Math.round(savedVol * 100);
    if (volVal) volVal.textContent = Math.round(savedVol * 100) + '%';
    volSlider.addEventListener('input', () => {
      _audioVolume = parseInt(volSlider.value) / 100;
      if (volVal) volVal.textContent = volSlider.value + '%';
      localStorage.setItem('tt_volume', String(_audioVolume));
    });
  }
  [D.optSmooth, D.optSound, D.optLiveWpm, D.optPunct, D.optNums, D.optCustomLoop,
  ...
  D.btnResetSettings?.addEventListener('click', () => {
    if (!confirm('Reset all settings to their defaults? Your history and scores are kept.')) return;
    // F: Remove only settings/theme/volume — leave history, profile, and pro untouched
    localStorage.removeItem(LS_SET);
    localStorage.removeItem('tt_theme');
    localStorage.removeItem('tt_volume');
    location.reload();
  });
