...
function cacheDOM() {
  const ids = [
    ...
    'btnDaily','btnGhost','btnAch','btnLB','btnStats','btnSet',
...
const PRO_FEATURES_DEF = [
  ...
  { ico:'📅', name:'Daily Bonuses',     sub:'Triple XP on daily challenges' },
...
function applySettings() {
  if (D.wordsWrap) D.wordsWrap.style.fontSize = S.fontSize + 'px';
  if (D.arena)     D.arena.classList.toggle('smooth-caret', S.smoothCaret);
  if (D.adTop)     D.adTop.classList.toggle('hidden', !S.showAds);
  if (D.adBot)     D.adBot.classList.toggle('hidden', !S.showAds);
  if (D.ghostToggleWrap) D.ghostToggleWrap.style.opacity = S.isPro ? '1' : '0.5';
