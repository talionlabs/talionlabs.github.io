...
const MAX_LEVEL   = 100;
const WPM_PER_LVL = 3.5;  // 350 WPM = Level 100
...
function positionLiveCursor() {
  ...
  const targetScroll = Math.max(0, Math.round((naturalTop - lineH) / lineH) * lineH);
...
function positionGhostCursor(charIdx) {
  ...
  // Apply same scroll offset as live cursor (Bug: arena scroll)
  gc.style.left   = `${eR.left - wR.left}px`;
  gc.style.top    = `${eR.top  - wR.top  + S.arenaScroll}px`;
...
const AC = {
  ...
  check(wpm, acc) {
    ...
    // Bug 6b: Broader suspicious score check
    if (wpm > 250 && acc >= 99) return { ok: false, why: 'Suspicious score' };
