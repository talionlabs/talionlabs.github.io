...
function renderChar(idx, state) {
  ...
  // BUG-FIX-1: Must use the real character (including plain space).
  // white-space:pre-wrap preserves spaces without collapse.
  // Using \u00A0 (non-breaking space) was preventing line-breaks at word
  // boundaries, so wordsTyped wrapped differently from wordsTarget on line 2+.
  ty.textContent = ch;
}
...
function addFriend(name, bestWpm) {
  const list = getFriends();
  if (list.find(f => f.name.toLowerCase() === name.toLowerCase())) return false;
  list.push({ name, bestWpm: bestWpm || null, addedAt: new Date().toISOString() });
  saveFriends(list);
  return true;
}

function removeFriend(name) {
  saveFriends(getFriends().filter(f => f.name !== name));
}

// Chat messages stored per friend pair (simple local simulation; replace with Firebase for real)
const LS_CHAT = 'tt_chat_';
function getChat(friendName) {
  try { return JSON.parse(localStorage.getItem(LS_CHAT + friendName) || '[]'); } catch { return []; }
}
function sendMessage(friendName, text) {
  const msgs = getChat(friendName);
  msgs.push({ from: 'you', text, ts: new Date().toISOString() });
  if (msgs.length > 200) msgs.splice(0, msgs.length - 200);
  localStorage.setItem(LS_CHAT + friendName, JSON.stringify(msgs));
}
...
function renderFriendsModal() {
  ...
  D.friendsBody.innerHTML = `
    ...
    <div class="friends-search">
      <input type="text" id="friendSearchInput" class="si" placeholder="Enter a player's display name…" maxlength="32" style="flex:1" />
      <button class="ctrl-btn accent small" id="btnAddFriend">Add Friend</button>
    </div>
    <div id="friendAddNote" style="font-size:.74rem;min-height:.9rem;margin-bottom:.5rem"></div>

    <!-- Tabs: My Friends | Friends Leaderboard | Chat (Pro) -->
    <div class="modal-tabs" style="margin-bottom:.75rem">
      <button class="tab active" id="ftabFriends">👥 Friends (${friends.length})</button>
      <button class="tab" id="ftabLB">🏆 Friends LB</button>
      <button class="tab" id="ftabChat">${isPro ? '💬 Chat' : '🔒 Chat'}</button>
    </div>
  ...
  renderTab('ftabFriends'); // default

  // Add friend logic
  document.getElementById('btnAddFriend')?.addEventListener('click', async () => {
    const note  = document.getElementById('friendAddNote');
    const input = document.getElementById('friendSearchInput');
    const name  = input?.value.trim();
    if (!name) { if(note){note.style.color='var(--err)';note.textContent='Enter a name.';} return; }

    // Check if they exist on global LB (optional if Firebase is ready)
    let bestWpm = null;
    if (S.fbDB) {
      try {
        const snap = await S.fbDB.collection(FB_COL.global).where('name','==',name).limit(1).get();
        if (!snap.empty) bestWpm = snap.docs[0].data().wpm || null;
      } catch {}
    }

    const added = addFriend(name, bestWpm);
    if (added) {
      if(note){note.style.color='var(--ok)';note.textContent=`✓ ${name} added!`;}
      if(input) input.value='';
      renderFriendsModal(); // refresh
    } else {
      if(note){note.style.color='var(--warn)';note.textContent=`${name} is already in your friends list.`;}
    }
  });
}
...
function renderFriendsList(container) {
  const friends = getFriends();
  if (!friends.length) {
    container.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--text-m);font-size:.86rem">No friends yet. Add one above!</div>';
    return;
  }
  container.innerHTML = `<div class="friends-list">${friends.map(f => `
    <div class="friend-row">
      ...
      <div class="friend-info">
        ...
        <div class="friend-wpm">${f.bestWpm ? f.bestWpm + ' WPM best' : 'No score recorded'}</div>
      </div>
      <div style="display:flex;gap:.4rem;margin-left:auto">
        <button class="ctrl-btn small" data-action="ghost" data-name="${esc(f.name)}" data-wpm="${f.bestWpm||0}" title="Race their WPM ghost">👻</button>
        <button class="ctrl-btn small danger" data-action="remove" data-name="${esc(f.name)}" title="Remove friend">✕</button>
      </div>
    </div>`).join('')}</div>`;
...
function renderFriendsLB(container) {
  ...
  const sorted = [...friends].filter(f => f.bestWpm).sort((a,b) => b.bestWpm - a.bestWpm);
  if (!sorted.length) {
    container.innerHTML = '<div style="text-align:center;padding:1rem;color:var(--text-m);font-size:.86rem">None of your friends have recorded WPM scores yet.</div>';
    return;
  }
  const medals = ['🥇','🥈','🥉'];
  container.innerHTML = `<table class="lb-table"><thead><tr><th>#</th><th>Name</th><th>WPM</th></tr></thead><tbody>
    ${sorted.map((f,i) => `
      <tr>
        ...
        <td class="lb-name">${esc(f.name)}</td>
        <td class="lb-wpm">${f.bestWpm}</td>
      </tr>`).join('')}
  </tbody></table>`;
}

function renderFriendsChat(container) {
  if (!S.isPro) {
    container.innerHTML = `
      <div style="text-align:center;padding:1.5rem">
        ...
        <div style="font-size:.84rem;color:var(--text-m);line-height:1.7;margin-bottom:1rem">
          Chat privately with friends.<br>Requires <b>TalionType Pro</b>.
        </div>
  ...
  // Show friend selector + chat window
  let activeFriend = friends[0].name;
  const render = () => {
    const msgs = getChat(activeFriend);
    container.innerHTML = `
      <div style="display:flex;gap:.4rem;margin-bottom:.65rem;flex-wrap:wrap">
        ${friends.map(f=>`<button class="pill ${f.name===activeFriend?'active':''}" data-chat="${esc(f.name)}">${esc(f.name)}</button>`).join('')}
      </div>
      <div id="chatMessages" style="height:160px;overflow-y:auto;background:var(--bg);border:1px solid var(--border-s);border-radius:10px;padding:.6rem .8rem;margin-bottom:.5rem;display:flex;flex-direction:column;gap:.35rem;font-size:.82rem">
        ${msgs.length ? msgs.map(m=>`
          <div style="display:flex;${m.from==='you'?'justify-content:flex-end':''};gap:.4rem">
            <div style="max-width:80%;padding:.35rem .6rem;border-radius:10px;${m.from==='you'?'background:var(--acc-dim);color:var(--acc)':'background:var(--bg-card);color:var(--text-m)'}">
              ${esc(m.text)}
              <span style="font-size:.62rem;color:var(--text-f);display:block;margin-top:1px">${new Date(m.ts).toLocaleTimeString()}</span>
            </div>
          </div>`).join('') : '<div style="color:var(--text-f);text-align:center;margin:auto">No messages yet. Say hello!</div>'}
      </div>
      <div style="display:flex;gap:.4rem">
        <input type="text" id="chatMsgInput" class="si" placeholder="Type a message…" style="flex:1" maxlength="200" />
        <button class="ctrl-btn accent small" id="btnSendChat">Send</button>
      </div>
      <p style="font-size:.68rem;color:var(--text-f);margin-top:.35rem">Messages are stored locally. For cross-device sync, connect Firebase.</p>`;
    ...
    container.querySelectorAll('[data-chat]').forEach(btn => {
      btn.addEventListener('click', () => { activeFriend = btn.dataset.chat; render(); });
    });
    ...
    const sendMsg = () => {
      ...
      if (!txt) return;
      sendMessage(activeFriend, txt);
      if (inp) inp.value = '';
      render();
    };
    container.querySelector('#btnSendChat')?.addEventListener('click', sendMsg);
    container.querySelector('#chatMsgInput')?.addEventListener('keydown', e => { if (e.key==='Enter') sendMsg(); });
  };
  render();
}
...
async function confirmSave() {
  ...
  if (S.fbReady && S.currentUser) {
    // BUG-FIX-7: Enforce global leaderboard username uniqueness
    if (D.saveNote) D.saveNote.textContent = 'Checking username availability…';
    const available = await isNameAvailable(name);
    if (!available) {
      if (D.saveNote) { D.saveNote.style.color = 'var(--err)'; D.saveNote.textContent = '⚠ That name is already taken by another player. Choose a different display name.'; }
      return; // do NOT close modal — let user change the name
    }
    if (D.saveNote) { D.saveNote.style.color = ''; D.saveNote.textContent = ''; }
